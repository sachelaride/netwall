...
class RemoteAccessModule {
    ...
    setupHandlers() {
        ...
        this.socket.on('access-request', async (data) => {
        ...
        this.socket.on('user-input', async (input) => {
            if (this.permissionMode !== 'administrator')
                return;
            try {
    ...
    stopVNCServer() {
        ...
        // Close Firewall Port
        (0, child_process_1.exec)(`powershell -Command "Remove-NetFirewallRule -DisplayName 'Netwall VNC'"`, (err) => {
            if (err)
                console.error('[Remote] Failed to remove firewall rule:', err);
        });
    }
}
