...
export function TopologyMap() {
    ...
    const { nodes: realNodes, links: realLinks } = useMemo(() => {
        const nodes: Node[] = [
            ...
            { id: 'FW1', type: 'firewall', label: 'Gateway', x: 400, y: 150, status: 'ok', ip: '192.168.0.254' },
            { id: 'SW_B01_CORE', type: 'switch', label: 'Core Switch (B01)', x: 400, y: 250, status: 'ok', ip: '192.168.0.1' },
        ];

        const links: Link[] = [
            { source: 'WAN', target: 'FW1' },
            { source: 'FW1', target: 'SW_B01_CORE' },
        ];

        // Filter devices if requested
        let filteredDevices = showOnlySwitches ? devices.filter(d => d.type === 'switch') : devices;

        // Deduplicate: Don't add scanned devices that have the same IP as static infra
        const staticIps = nodes.map(n => n.ip);
        filteredDevices = filteredDevices.filter(d => !staticIps.includes(d.ip));
        ...
        filteredDevices.forEach((device: any, index: number) => {
            ...
            const parentId = device.parentId || 'SW_B01_CORE';
                ...
                {selectedNode && (
                    <div className="absolute top-4 right-4 bg-slate-800/90 backdrop-blur border border-slate-700 p-4 rounded-lg w-64 shadow-xl">
                        ...
                        {(() => {
                            ...
                            const isStatic = ['WAN', 'FW1', 'SW_B01_CORE'].includes(node.id);
